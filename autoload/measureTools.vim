...
function! measureTools#getStep(...) abort
	let [ offset, length, stride ] = map(copy(a:000), {_,val -> eval(val)})
	let calc = [
	...
	while point < length
		call add(calc, printf('  %2d:%8.1f', index, point))
		let point += stride
		let index += 1
	endwhile
...
function! measureTools#toPolar(...) abort
	let [ width, height ] = map(copy(a:000), {_,val -> eval(val)})
	let calc = [
...
function! measureTools#toRect(...) abort
	let [ range, angle ] = map(copy(a:000), {_,val -> eval(val)})
	let calc = [
		\ printf('%7s: %6.1f', 'range', range),
		\ printf('%7s: %6.1fﾟ', 'angle', angle),
		\ '-----------------'
		\ ]
	call add(calc, printf('%7s: %7.2f', 'width', range*(cos(s:toRad(angle*1.0)))))
	call add(calc, printf('%7s: %7.2f', 'height', range*(sin(s:toRad(angle*1.0)))))
